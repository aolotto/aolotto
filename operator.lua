...
Handlers.add(
  "op.gen_winners_str",
  Handlers.utils.hasMatchingTag("Action","GenWinnersString"),
  function (msg)
    ...
    if msg.From == ao.id then
      ...
      local winners = ARCHIVES.repo[msg.Round].winners
      local str = string.format('@everyone: The winners of AOLOTTO*ALT Round %s :\n',msg.Round or " ")
      local idx = 1
      if winners and #winners > 0 then
        for index, value in spairs(winners, function(t,a,b) return t[b].rewards < t[a].rewards end ) do
          local line = tostring(idx).."."..value.id.." -> "..string.format('%.3f $ALT',value.rewards/1000).."\n"
          str = str..line
          idx = idx + 1
        end
      else
